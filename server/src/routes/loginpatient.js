...
let loginpatient = Router();


loginpatient.use(morgan('combined'));
loginpatient.use(bodyParser.json());
loginpatient.use(cors());
...
loginpatient.post('', async (req, res) => {
    ...
    let args = [req.body];
    let invokeResponse = await network.invoke(networkObj, true, 'getpatientDatapatient', args);
